...
#############
z = val_eval_lookahead[t, 0:X+1, 0:X+1]-val_dp_queue[t, 0:X+1, 0:X+1]  # Slice to exclude zeros for x1 and x2
# Plot the surface
fig = plt.figure(figsize=(10, 7))
ax = fig.add_subplot(111, projection='3d')
...
#################
t=100
derivative_x_2 = np.diff(val_fluid_queue[t][30], n=1)
plt.plot(derivative_x_2)
derivative_x_2_dp = np.diff(val_dp_queue[t][30], n=1)
plt.plot(derivative_x_2_dp)
derivative_x_2_lookahead = np.diff(val_lookahead[t][30], n=1)
plt.plot(derivative_x_2_lookahead)
derivative_x_2_eval_lookahead = np.diff(val_eval_lookahead[t][30], n=1)
plt.plot(derivative_x_2_eval_lookahead)
plt.show()

derivative_x_1 = np.diff(val_fluid_queue[t][:,30], n=1)
plt.plot(derivative_x_1)
derivative_x_1_dp = np.diff(val_dp_queue[t][:, 30], n=1)
plt.plot(derivative_x_1_dp)
derivative_x_1_lookahead = np.diff(val_lookahead[t][:,30], n=1)
plt.plot(derivative_x_1_lookahead)
derivative_x_1_eval_lookahead = np.diff(val_eval_lookahead[t][:,30], n=1)
plt.plot(derivative_x_1_eval_lookahead)
plt.show()

plt.plot(np.diff(val_lookahead[t][30], n=1)-np.diff(val_eval_lookahead[t][30], n=1))
plt.show()
...
ax = fig.add_subplot(111, projection='3d')
surf = ax.plot_surface(x1, x2, z, cmap='viridis')
surf2 = ax.plot_surface(x1, x2, z2, cmap='viridis')
# Add labels and title
ax.set_xlabel('x1')
ax.set_ylabel('x2')
ax.set_zlabel('Difference Cost')
